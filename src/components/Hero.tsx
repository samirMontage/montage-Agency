...
import { MEDIA, SITE, STATS, CONTACT } from "../data/content";
...
export function Hero() {
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute right-[-8%] top-1/2 hidden -translate-y-1/2 opacity-45 md:block lg:right-[2%] lg:opacity-60">
      ...
      {/* Статистика */}
      <div className="absolute inset-x-0 bottom-0 z-10 border-t border-white/10 bg-ink/55 backdrop-blur-md">
        <div className="mx-auto grid max-w-4xl grid-cols-3 divide-x divide-white/10">
          {STATS.map((stat, i) => (
            <Reveal key={stat.label} delay={420 + i * 110}>
              <div className="flex flex-col items-center gap-1.5 px-2 py-5 sm:py-7">
                <span className="text-shimmer text-xl font-semibold tracking-tight sm:text-3xl">
                  {stat.value}
                </span>
                <span className="text-[9px] uppercase tracking-[0.18em] text-white/50 sm:text-[11px] sm:tracking-[0.22em]">
                  {stat.label}
                </span>
              </div>
            </Reveal>
